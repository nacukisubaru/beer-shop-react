...
import { DataGrid, getGridStringOperators, GridSortItem, GridToolbar } from "@mui/x-data-grid";
...
const TableGrid:FC<ITableGridProps> = ({columns, rows, pageSize}) => {
    ...
    return (
        <div style={{ height: 860, width: "100%", marginBottom: "10px"}}>
            ...
            <DataGrid
                rows={rows}
                columns={columns.map((item)=>{return {...item, filterOperators}})}
                pageSize={pageSize}
                rowsPerPageOptions={[]}
                ...
                sortModel={sortModel}
                components={{Footer: PaginationGrid, Toolbar: GridToolbar }}
                onFilterModelChange={()=>{console.log('we cand send request here!')}}
                localeText={{filterOperatorEquals:'Равняется', filterOperatorContains: 'Содержит в себе'}}
            />
        </div>
    );
};
